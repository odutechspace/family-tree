...
const TYPE_STYLES: Record<string, string> = {
  ...
  daily:
    "border-blue-300 bg-blue-50/90 dark:border-blue-700/50 dark:bg-blue-950/20",
  weekly:
    "border-purple-300 bg-purple-50/90 dark:border-purple-700/50 dark:bg-purple-950/20",
  discovery:
    "border-amber-300 bg-amber-50/90 dark:border-amber-700/50 dark:bg-amber-950/20",
};
...
const TYPE_BADGE: Record<string, string> = {
  ...
  weekly:
    "bg-purple-200 text-purple-900 dark:bg-purple-900/50 dark:text-purple-300",
  discovery:
...
export default function QuestCard({
  ...
  return (
    <div
      className={`rounded-xl border p-4 transition ${cardStyle} ${isCompleted ? "opacity-70" : ""}`}
    >
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0 flex-1">
          <div className="mb-1 flex flex-wrap items-center gap-2">
            ...
            <span
              className={`rounded-full px-1.5 py-0.5 text-xs capitalize ${badgeStyle}`}
            >
              {type}
            </span>
            {isCompleted && (
              <span className="text-xs font-medium text-emerald-600 dark:text-green-400">
                ✓ Done
              </span>
            )}
          </div>
