...
export async function GET(_req: NextRequest) {
  ...
  const user = await repo.findOne({
    where: { id: auth.id },
    select: ["id", "name", "email", "role", "profilePhotoUrl", "linkedPersonId", "createdAt", "updatedAt"],
  });
...
export async function PATCH(req: NextRequest) {
  ...
  if (linkedPersonId !== undefined) {
    if (linkedPersonId === null) {
      ...
      const person = await AppDataSource.getRepository(Person).findOne({ where: { id: pid } });
      if (!person) return apiError(ApiError.badRequest("That person does not exist."));
