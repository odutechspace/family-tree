...
export async function POST(req: NextRequest) {
  ...
  if (!currentPassword || !newPassword) {
    return apiError(ApiError.badRequest("Current password and new password are required."));
  }
  if (newPassword.length < 8) {
    return apiError(ApiError.badRequest("New password must be at least 8 characters."));
  }
  ...
  if (!valid) return apiError(ApiError.forbidden("Current password is incorrect."));
