...
export async function POST(req: NextRequest) {
  ...
  if (!personAId || !personBId || !type) {
    return apiError(ApiError.badRequest("personAId, personBId and type are required."));
  }
  if (personAId === personBId) {
    return apiError(ApiError.badRequest("A person cannot have a relationship with themselves."));
  }
  ...
  const gamification = await awardXP(user.id, XPEventType.ADD_RELATIONSHIP, (saved as any).id, `Linked ${type} relationship`);

  return apiSuccess({ relationship: saved, gamification }, "Relationship created", 201);
}
