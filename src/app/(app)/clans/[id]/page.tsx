...
export default function ClanDetailPage() {
  ...
  if (!clan) {
    return (
      <div className="flex min-h-screen items-center justify-center bg-background text-destructive">Clan not found.</div>
    );
  }
      ...
      <div className="mx-auto max-w-4xl">
        ...
        <div className="mb-6 rounded-2xl border border-primary/20 bg-gradient-to-r from-primary/10 to-card p-8">
          <div className="flex items-start gap-6">
            <div className="flex h-20 w-20 flex-shrink-0 items-center justify-center rounded-xl border border-primary/30 bg-primary/15 text-4xl">🦁</div>
            <div className="flex-1">
              ...
              {clan.alternateName && <p className="mb-3 text-lg text-primary/80">{clan.alternateName}</p>}
            ...
            {clan.praisePoem && (
              <Card>
                <CardContent className="p-6">
                  <h2 className="mb-4 font-semibold text-primary">Praise Poem / Izibongo / Oriki</h2>
                  <p className="font-serif text-lg italic leading-relaxed text-foreground/90 whitespace-pre-wrap">{clan.praisePoem}</p>
                </CardContent>
              </Card>
            )}
            {clan.history && (
              <Card>
                <CardContent className="p-6">
                  <h2 className="mb-4 font-semibold text-primary">Clan History</h2>
                  <p className="leading-relaxed text-foreground whitespace-pre-wrap">{clan.history}</p>
                </CardContent>
              </Card>
            )}
          </div>
          ...
          <div>
            <Card>
              <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                <CardTitle className="text-base text-primary">Members ({members.length})</CardTitle>
              ...
              <CardContent>
                {members.length === 0 ? (
                  <p className="text-sm text-muted-foreground">No members yet. Add people with this clan assigned.</p>
                ) : (
