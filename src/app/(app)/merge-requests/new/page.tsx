...
function NewMergeRequestForm() {
  ...
  }>({ source: [], target: [], connecting: [], sourceTree: [], targetTree: [] });
  ...
  const searchTrees = async (field: "sourceTree" | "targetTree", q: string) => {
    ...
    setResults((r) => ({
      ...r,
      [field]: all.filter((t: FamilyTree) => t.name.toLowerCase().includes(q.toLowerCase())),
    }));
  };
          ...
          {results[field].map((p: Person) => (
            <button
              ...
              onClick={() => {
                ...
                setLabels((l) => ({ ...l, [field]: `${p.firstName} ${p.lastName}` }));
                setResults((r) => ({ ...r, [field]: [] }));
              }}
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError("");
    try {
      const body: Record<string, unknown> = { type: mergeType, reason: form.reason, evidenceNotes: form.evidenceNotes };
    ...
    <div className="min-h-screen bg-background px-4 py-8 text-foreground">
      <div className="mx-auto max-w-xl">
        ...
        <div className="mb-6 rounded-xl border border-primary/25 bg-primary/5 p-4 text-sm text-foreground">
          ...
          <br />
          Use this when you find duplicate person profiles for the same real person, or when two separate family trees
          actually belong to the same extended family. An admin will review and approve the merge.
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <Card>
            <CardContent className="space-y-3 p-5">
              ...
              <div className="grid grid-cols-2 gap-3">
                <Button
                  type="button"
                  variant={mergeType === "duplicate_person" ? "default" : "outline"}
          ...
          <Card>
            <CardContent className="space-y-4 p-5">
              {mergeType === "duplicate_person" ? (
                <>
                  <h3 className="font-medium text-primary">Select the two duplicate person profiles</h3>
                ...
                <>
                  <h3 className="font-medium text-primary">Select the two family trees</h3>
                  <div className="space-y-2">
                    <Label>Source Tree (to merge from)</Label>
                    <Input
                      value={searches.sourceTree}
                      onChange={(e) => setSearches((s) => ({ ...s, sourceTree: e.target.value }))}
                      ...
                      <div className="mt-1 rounded-md border border-border bg-popover">
                        {results.sourceTree.map((t: FamilyTree) => (
                          <button
                            ...
                            type="button"
                            onClick={() => {
                              setForm((f) => ({ ...f, sourceTreeId: String(t.id) }));
                              setSearches((s) => ({ ...s, sourceTree: t.name }));
                  ...
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      onChange={(e) => setSearches((s) => ({ ...s, targetTree: e.target.value }))}
                      placeholder="Search your trees..."
                    />
                      ...
                      <div className="mt-1 rounded-md border border-border bg-popover">
                        {results.targetTree.map((t: FamilyTree) => (
                          <button
                            ...
                            type="button"
                            onClick={() => {
                              setForm((f) => ({ ...f, targetTreeId: String(t.id) }));
                              setSearches((s) => ({ ...s, targetTree: t.name }));
          ...
          <Card>
            <CardHeader>
              <CardTitle className="text-base text-primary">Justification</CardTitle>
            </CardHeader>
