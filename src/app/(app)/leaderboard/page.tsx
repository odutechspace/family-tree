      ...
      <div className="max-w-3xl mx-auto">
        ...
        <div className="mb-8">
          ...
          <p className="text-stone-400 mt-1">
            Who is building the richest family heritage?
          </p>
        </div>
        ...
        {!loading && leaderboard.length >= 3 && (
          <div className="flex items-end justify-center gap-4 mb-10">
            ...
            {/* 1st */}
            <PodiumCard tall entry={leaderboard[0]} myId={user?.id} />
            {/* 3rd */}
        ...
        {myRank && (
          <div className="mb-4 bg-amber-900/20 border border-amber-700/50 rounded-xl p-4 flex items-center gap-4">
            ...
            <div className="flex-1">
              ...
              <p
                className={`text-xs ${LEVEL_COLORS[myRank.level - 1] || "text-stone-400"}`}
              >
            ...
            <div className="text-right">
              ...
              {myRank.currentStreak > 0 && (
                <p className="text-orange-400 text-xs">
                  🔥 {myRank.currentStreak} day streak
                </p>
              )}
            </div>
          </div>
        ...
        {loading ? (
          <div className="space-y-2">
            {Array.from({ length: 10 }).map((_, i) => (
              <div
                key={i}
                className="h-16 bg-stone-800 rounded-xl animate-pulse"
              />
            ))}
          </div>
        ) : (
          <div className="space-y-2">
            {leaderboard.map((entry) => {
              ...
              const lvlColor =
                LEVEL_COLORS[
                  Math.min(entry.level - 1, LEVEL_COLORS.length - 1)
                ];
              ...
              return (
                ...
                >
                  ...
                  <div
                    className={`w-10 h-10 rounded-full flex items-center justify-center text-white font-bold text-sm flex-shrink-0 ${isMe ? "bg-amber-700" : "bg-stone-700"}`}
                  >
                  ...
                  <div className="flex-1 min-w-0">
                    <p
                      className={`font-semibold truncate ${isMe ? "text-amber-400" : "text-white"}`}
                    >
                      {entry.name} {isMe && "(you)"}
                    </p>
...
function PodiumCard({
  ...
  return (
    <div
      className={`flex flex-col items-center gap-2 flex-1 max-w-32 ${tall ? "pb-0" : "pb-4"}`}
    >
      <span className="text-3xl">{medal}</span>
      <div
        className={`w-14 h-14 rounded-full flex items-center justify-center font-bold text-lg border-2 ${isMe ? "border-amber-400 bg-amber-900/40 text-amber-300" : "border-stone-600 bg-stone-700 text-white"}`}
      >
        {entry.name[0]}
      </div>
      <p
        className={`text-xs font-semibold text-center truncate w-full ${isMe ? "text-amber-400" : "text-white"}`}
      >
        {entry.name}
      </p>
      ...
      <div
        className={`w-full rounded-t-lg ${tall ? "h-20 bg-amber-800/30 border-t-2 border-amber-600" : "h-12 bg-stone-800 border-t border-stone-600"}`}
      />
    </div>
  );
}
